...
def federate(utils):
		...
		for i in range(1, len(client_models)):
				...
				for k in client_dict.keys():
						# print("before:",server_new_dict[k], server_new_dict[k].shape)
						server_new_dict[k] += client_dict[k]
						# print("after:",server_new_dict[k],server_new_dict[k].shape)

		for k in server_new_dict.keys():
				server_new_dict[k] = server_new_dict[k] / n
				# print(server_new_dict[k],server_new_dict[k].shape)
...
class FederatedNCF:
	def __init__(self, ui_matrix, num_clients=50, user_per_client_range=[1, 5], mode="ncf", aggregation_epochs=50, local_epochs=10, batch_size=128, latent_dim=32, seed=0):
		...
		# self.single_round_results = {}#1106
	...
	def plot_loss_curve(self, loss_values):#1106
		epochs = range(1, len(loss_values) + 1)

		plt.plot(epochs, loss_values, 'b', label='Training Loss')
		plt.title('Training Loss')
		plt.xlabel('Aggregation Epochs')
		plt.ylabel('Loss')
		plt.legend()
		plt.show()
	...
	def single_round(self, epoch=0, first_time=False):
		...
		for client_id, client in bar:
			...
			# loss_values.append(results['loss'])#1106
		...
		bar.close()
		# self.plot_loss_curve(self.single_round_results['loss']) #1106
	...
	def train(self):
		...
		for epoch in range(self.aggregation_epochs):
			...
			# loss_values.append(self.single_round_results['loss'])#1106
			first_time = False
...
if __name__ == '__main__':
	...
	fncf = FederatedNCF(dataloader.ratings, num_clients=2, user_per_client_range=[1, 10], mode="ncf", aggregation_epochs=100, local_epochs=1, batch_size=128)
