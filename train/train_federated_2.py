...
def federate(utils):
  ...
  # server_param_list = server_new_dict[k].cpu().flatten()
  # print(server_new_dict[k], "s")
  # result=np.array(server_param_list)
  # np.savetxt(r"output_server{}.txt".format(cnt),result)
  # cnt+=1

  # print(server_param_list)
  # exit()
  # server_encrypted_param = [public_key.encrypt(val) for val in server_param_list]
  # server_new_dict[k] = server_new_dict[k] + server_encrypted_param
  # server_new_dict[k] = server_encrypted_param
  cnt = 0
  for i in range(1, len(client_models)):
    ...
    for k in client_dict.keys():
      ...
      print(client_encrypted_param)
      # end_time = time.time()
      #
      # training_time = end_time - start_time
      # print("encryption time: ", training_time)
      ...
      # training_time = end_time - start_time
      # print("decryption time: ", training_time)
      # exit()
      # print(len(server_decrypted_param_list))
      # server_new_dict[k] = torch.tensor(new_server_param_list).reshape(server_model.state_dict()[k].shape)
      server_new_dict[k] = torch.tensor(server_decrypted_param_list).reshape(server_model.state_dict()[k].shape)
...
# 	fncf = FederatedNCF(dataloader.ratings, num_clients=2, user_per_client_range=[1, 10], mode="ncf", aggregation_epochs=20, local_epochs=10, batch_size=128)
# 	fncf.train()

if __name__ == '__main__':
  dataloader = MovielensDatasetLoader()

  fncf = FederatedNCF(dataloader.ratings, num_clients=2, user_per_client_range=[1, 10], mode="ncf", aggregation_epochs=1, local_epochs=10,
                      batch_size=128)
